...
public_version: true
generated: "2026-06-12T14:24:50.827142+00:00"
---
...
| unsupervised | recurring term detected from title/metadata/PDF text |
| depth | recurring term detected from title/metadata/PDF text |
| demo | recurring term detected from title/metadata/PDF text |
| event | recurring term detected from title/metadata/PDF text |
| learning | recurring term detected from title/metadata/PDF text |
| egomotion | recurring term detected from title/metadata/PDF text |
| cvpr | recurring term detected from title/metadata/PDF text |
| network | recurring term detected from title/metadata/PDF text |

## Chinese Reading Note

这篇论文由 Alex Zihao Zhu et al. 发表，主题集中在 event-based imaging, motion restoration。脚本已按 nature-reader 的 source-map-first 思路读取本地 PDF，并用章节位置、术语表和元数据生成公开版阅读笔记；由于目标仓库是公开 GitHub Pages，本文件不包含论文全文原文、全文中文翻译或完整图表复刻。
...
- 公开版仅记录方法线索，不复写论文正文。
- 高频术语：optical, flow, unsupervised, depth, demo, event, learning, egomotion。
...
- Zotero item key: `5CJPVYTU`
- Attachment key: `CWSCERZL`
- DOI: 10.1109/CVPRW.2019.00216
- URL: Not recorded
